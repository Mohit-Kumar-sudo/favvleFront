...
class Library extends React.Component {
  ...
  componentDidMount() {
      ...
      .then((response) => {
        ...
        if (response.data.status) {
          ...
          if (response.data.profile) {
            this.setState({
              image:
                process.env.NEXT_PUBLIC_BASE_URL + "/" + response.data.profile,
            });
          }
    ...
    axios
      .get(process.env.NEXT_PUBLIC_BASE_URL + "/admin/gettexts", {
        params: { page: "library" },
      })
      .then((response) => {
  ...
  handleImage = (event) => {
    ...
    axios
      .post(
        process.env.NEXT_PUBLIC_BASE_URL + "/users/updateprofilepic",
        form,
        {
          headers: {
            authorization: `Bearer ${usertoken}`,
                    ...
                    <a
                      ...
                      onClick={() => {
                        if (this.state.ranklist.length > 4)
                          ...
                          this.props.router.push({ pathname: "/Categories" });
                      }}
                    >
                      Create New List
                    </a>
